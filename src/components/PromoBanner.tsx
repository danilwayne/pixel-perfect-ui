...
const PromoBanner = () => {
  const { elementRef, backgroundImage } = useDeferredBackground<HTMLElement>(promoBanner);

  return (
    <section
      ref={elementRef}
      id="special-offer" // id da secao para navegacao
      className="relative py-24 md:py-28 bg-pet-dark" // fallback solido enquanto a foto ainda nao foi carregada
      style={{
        // Revisao de performance:
        // esta imagem tem mais de 1 MB. Agora ela so entra quando a secao estiver perto da viewport.
        backgroundImage,
